...
class Slotmaster extends Model
{
    ...
    public function getslotdetails($esteblishmentUserMapID)
    {
        ...
        try {
            ...
            if ($weekdays == 0)
                $weekdays = 7;
            if (isset($clientID)) {
                $slotdataArray = Slotmaster::where('user_map_id', $esteblishmentUserMapID)->where('clinicID', $clientID)->where('day_id', $weekdays)->get();
            } else {
    ...
    public function getslotdetailsV3($esteblishmentUserMapID, $date = null, $clientID = null)
    {

        try {
            $date = $date ?? ($_GET['date'] ?? date('Y-m-d'));
            $clientID = $clientID ?? ($_GET['clinicID'] ?? null);
            ...
            if (isset($date) && $date != date('Y-m-d')) {
                $date = new DateTime(date('Y-m-d H:i', strtotime($date)));
            } else {
                $date = new DateTime(date('Y-m-d H:i'));
            }
            $appt = new AppointmentDetails();
            if (isset($date)) {
                $bookedslotdata = $appt->getallslotV3($esteblishmentUserMapID, $date);
            } else {
